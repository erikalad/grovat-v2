...
export const fetchData = (usuario, contraseña) => {
  return async (dispatch) => {
    try {
      ...
      if (
        usuarioLogueadoString &&
        usuario === userLogueado &&
        contraseña === contraLogueada
      ) {
        ...
        response = await axios.get(
          `https://meicanalitycs.onrender.com/clienteid/${clienteId}`
        );
      } else {
        response = await axios.post(
          "https://meicanalitycs.onrender.com/login",
          { usuario, contraseña }
        );

        // Filtrar el array de usuarios para encontrar el usuario con el que se ha iniciado sesión
        const usuarioLogueado = response.data.usuarios.find(
  ...
  return async (dispatch) => {
    try {
      ...
      const response = await axios.patch(
        "https://meicanalitycs.onrender.com/usuario/" + idUsuario,
        { logueado: false, email: email, usuario: user }
      );
  ...
  return async (dispatch) => {
    try {
      ...
      const response = await axios.post(
        // "https://meicanalitycs.onrender.com/mensaje",
        "http://localhost:3001/mensaje",
        { mensajes, nombre }
      );
      // Verificar si la solicitud fue exitosa
...
export const editUser = (campos) => {
  return async (dispatch) => {
    try {
      // Enviar solicitud POST para desloguear al usuario
      const response = await axios.patch(
        `https://meicanalitycs.onrender.com/usuario/${campos.id}`,
        campos.userActualizado
      );
...
export const editUsuarioCliente = (campos) => {
  return async (dispatch) => {
    try {
      // Enviar solicitud POST para desloguear al usuario
      const response = await axios.patch(
        `https://meicanalitycs.onrender.com/usuario/${campos.id_usuario}`,
        campos
      );
      // Verificar si la solicitud fue exitosa
      if (response.status === 200) {
...
export const addUser = (campos) => {
  return async (dispatch) => {
    try {
      ...
      const response = await axios.post(
        "https://meicanalitycs.onrender.com/usuario",
        campos
      );
...
export const postFuncionalidades = (campos) => {
  return async (dispatch) => {
    try {
      // Realizar la petición POST con Axios
      const response = await axios.post(
        "https://meicanalitycs.onrender.com/funcionalidades",
        campos
      );
...
export const postCustomizaciones = (campos) => {
  return async (dispatch) => {
    try {
      // Realizar la petición POST con Axios
      const response = await axios.post(
        "https://meicanalitycs.onrender.com/customizaciones",
        campos
      );
...
export const getClientes = () => {
  return async (dispatch) => {
    try {
      const response = await axios.get(
        "https://meicanalitycs.onrender.com/clientes"
      );
...
export const postCliente = (campos) => {
  return async (dispatch) => {
    try {
      // Realizar la petición POST con Axios
      const response = await axios.post(
        "https://meicanalitycs.onrender.com/cliente",
        campos
      );
...
export const patchFuncionalidades = (campos, id) => {
  return async (dispatch) => {
    try {
      const response = await axios.patch(
        "https://meicanalitycs.onrender.com/funcionalidades/" + id,
        campos
      );
...
export const patchCliente = (campos, id) => {
  return async (dispatch) => {
    try {
      const response = await axios.patch(
        "https://meicanalitycs.onrender.com/cliente/" + id,
        campos
      );
